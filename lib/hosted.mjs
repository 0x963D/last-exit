...
export function makeHandler({sql=process.env.DATABASE_URL?neon(process.env.DATABASE_URL):null,key=process.env.TYPESAFE_API_KEY,secret=process.env.SESSION_SECRET,budgetId=process.env.VERCEL_ENV==='production'?'public-v1':'preview-v1',query=queryJev}={}){
  return async(req,res)=>{
    ...
    try{
      ...
      if(!['/api/start','/api/turn','/api/visit'].includes(path))return send(404,{error:'Not found.'});
      let data=req.body;
      ...
      if(path==='/api/visit'){
        const [{allowed}]=await sql`SELECT last_exit.rate(${`visit:${actor}:${bucket}`},60) AS allowed`;
        if(allowed)await sql`SELECT last_exit.count_usage(${budgetId},'page_view')`;
      ...
      if(path==='/api/start'){
        ...
        await sql.transaction([
          sql`INSERT INTO last_exit.sessions(id,state,expires_at) VALUES(${game.id}::uuid,${JSON.stringify(game)}::jsonb,now()+interval '4 hours')`,
          sql`SELECT last_exit.count_usage(${budgetId},'started')`
        ]);
